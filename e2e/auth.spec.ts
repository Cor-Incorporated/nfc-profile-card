...
test.describe("認証フロー", () => {
  ...
  test("メールアドレスでの新規登録フロー", async ({ page }) => {
    ...
    const submitButton = page.getByRole("button", {
      name: /登録|sign up|create account/i,
    });
  ...
  test("メールアドレスでのサインインフロー", async ({ page }) => {
    ...
    const errorMessage = page.locator(
      "text=/アカウントが見つかりません|User not found|Invalid credentials/i",
    );
  ...
  test("パスワードリセットフロー", async ({ page }) => {
    ...
    const forgotPasswordLink = page.locator(
      "text=/パスワードを忘れた|Forgot password/i",
    );
    if (await forgotPasswordLink.isVisible()) {
      ...
      await expect(
        page.locator(
          "text=/メールを送信しました|Email sent|Check your email/i",
        ),
  ...
  test("Googleサインインボタンの存在確認", async ({ page }) => {
    ...
    const googleButton = page.locator(
      'button:has-text("Google"), button:has-text("Googleでサインイン")',
    );
  ...
  test("認証エラーメッセージの表示", async ({ page }) => {
    ...
    // エラーメッセージが表示されることを確認
    const errorMessage = page.locator(
      "text=/メールアドレスの形式|Invalid email|パスワード/i",
    );
...
test.describe("認証後のナビゲーション", () => {
  ...
  test("サインアウト機能", async ({ page }) => {
    ...
    const signOutButton = page.locator(
      'button:has-text("サインアウト"), button:has-text("Sign Out"), button:has-text("Logout")',
    );
