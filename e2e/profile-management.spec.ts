...
test.describe('プロフィール管理', () => {
  ...
  test('ダッシュボードの基本要素確認', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').first()).toContainText(/ダッシュボード|Dashboard|マイページ/i)

    // プロフィール編集リンクの存在確認
    const editProfileLink = page.locator('a[href="/dashboard/edit"], button:has-text("プロフィール編集")')
  ...
  test('プロフィール編集ページへの遷移', async ({ page }) => {
    // プロフィール編集リンクをクリック
    const editLink = page.locator('a[href="/dashboard/edit"], button:has-text("プロフィール編集")').first()
    await editLink.click()

    // 編集ページへの遷移を確認
    await expect(page).toHaveURL('/dashboard/edit')
    await expect(page.locator('h1, h2').first()).toContainText(/プロフィール編集|Edit Profile|プロフィール設定/i)
  })
  ...
  test('プロフィール情報の更新', async ({ page }) => {
    ...
    const bioInput = page.locator('textarea[name="bio"], textarea[placeholder*="自己紹介"]').first()
    await bioInput.fill('これはテスト用の自己紹介文です。')

    const companyInput = page.locator('input[name="company"], input[placeholder*="会社"]').first()
    if (await companyInput.isVisible()) {
      await companyInput.fill('テスト株式会社')
    }

    const positionInput = page.locator('input[name="position"], input[placeholder*="役職"]').first()
    ...
    const successMessage = page.locator('text=/保存しました|Saved successfully|更新しました/i')
    const dashboardRedirect = page.waitForURL('/dashboard', { timeout: 5000 }).catch(() => null)
  ...
  test('ソーシャルリンクの追加と削除', async ({ page }) => {
    ...
    // リンク追加ボタンを探してクリック
    const addLinkButton = page.locator('button:has-text("リンク"), button:has-text("Add Link"), button:has-text("追加")')

    if (await addLinkButton.first().isVisible()) {
      await addLinkButton.first().click()

      // 新しいリンク入力フィールドを探す
      const linkInputs = page.locator('input[placeholder*="URL"], input[placeholder*="https://"]')
      ...
      // タイトル入力（存在する場合）
      const titleInput = page.locator('input[placeholder*="タイトル"], input[placeholder*="Title"]').last()
      ...
      // 成功メッセージを確認
      await expect(page.locator('text=/保存|Success/i')).toBeVisible({ timeout: 5000 })
    }
  })
  ...
  test('プロフィール画像のアップロード', async ({ page }) => {
    ...
    if (await fileInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const preview = page.locator('img[alt*="プロフィール"], img[alt*="Profile"], img[alt*="Avatar"]')
  ...
  test('ユーザー名の変更と一意性チェック', async ({ page }) => {
    ...
    const usernameInput = page.locator('input[name="username"], input[placeholder*="ユーザー名"]').first()
    ...
    if (await usernameInput.isVisible()) {
      ...
      const availableMessage = page.locator('text=/利用可能|Available|使用できます/i')
      const errorMessage = page.locator('text=/使用中|Already taken|利用できません/i')

      await expect(availableMessage.or(errorMessage).first()).toBeVisible({ timeout: 5000 })
    }
  })
...
test.describe('公開プロフィールページ', () => {
  test('公開プロフィールページの表示', async ({ page }) => {
    ...
    await expect(profileName.or(notFound).first()).toBeVisible({ timeout: 5000 })
  })
  ...
  test('VCard ダウンロード機能', async ({ page }) => {
    ...
    // VCardダウンロードボタンを探す
    const vcardButton = page.locator('button:has-text("連絡先"), button:has-text("Download"), button:has-text("VCard")')

    if (await vcardButton.first().isVisible({ timeout: 5000 }).catch(() => false)) {
      // ダウンロードイベントをリッスン
  ...
  test('ソーシャルリンクの表示と動作', async ({ page }) => {
    ...
    // ソーシャルリンクセクションを探す
    const linksSection = page.locator('section').filter({ has: page.locator('a[href*="http"]') })
...
test.describe('NFCカード管理', () => {
  ...
  test('NFCカードの追加', async ({ page }) => {
    // NFCカード管理セクションを探す
    const nfcSection = page.locator('section').filter({ hasText: /NFCカード|NFC Card/i })

    if (await nfcSection.isVisible()) {
      // カード追加ボタンをクリック
      const addCardButton = nfcSection.locator('button:has-text("追加"), button:has-text("Add Card")')
      if (await addCardButton.isVisible()) {
        await addCardButton.click()

        // モーダルまたは新規ページの表示を確認
        const cardIdInput = page.locator('input[placeholder*="カードID"], input[placeholder*="Card ID"]')
        ...
        // カード名を入力（オプショナル）
        const cardNameInput = page.locator('input[placeholder*="カード名"], input[placeholder*="Card Name"]')
        if (await cardNameInput.isVisible()) {
          await cardNameInput.fill('テストカード')
        }

        // 保存
        const saveButton = page.getByRole('button', { name: /保存|Save|登録/i })
        await saveButton.click()

        // 成功メッセージを確認
        await expect(page.locator('text=/登録しました|Added successfully/i')).toBeVisible({ timeout: 5000 })
      }
    }
  })
  ...
  test('NFCカードの無効化', async ({ page }) => {
    // 既存のカードリストを探す
    const cardList = page.locator('[data-testid="card-list"], .card-list, ul').filter({ has: page.locator('text=/CARD-/i') })

    if (await cardList.isVisible({ timeout: 5000 }).catch(() => false)) {
      // 最初のカードの無効化ボタンをクリック
      const disableButton = cardList.locator('button:has-text("無効化"), button:has-text("Disable")').first()
      ...
      if (await disableButton.isVisible()) {
        ...
        // ステータス変更を確認
        await expect(page.locator('text=/無効|Disabled/i')).toBeVisible({ timeout: 5000 })
      }
    }
  })
})