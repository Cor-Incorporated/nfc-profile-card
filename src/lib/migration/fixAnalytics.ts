...
export async function migrateAnalyticsData() {
  ...
  for (const userDoc of snapshot.docs) {
    try {
      ...
      const calculatedTotal = Object.values(dailyViews).reduce(
        (a, b) => a + b,
        0,
      );
      ...
      if (actualTotal > calculatedTotal) {
        // 差分を今日の閲覧数として追加
        dailyViews[today] =
          (dailyViews[today] || 0) + (actualTotal - calculatedTotal);
        console.log(
          `User ${userDoc.id}: Adding ${actualTotal - calculatedTotal} views to today`,
        );
      }
