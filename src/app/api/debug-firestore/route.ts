...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (profileData?.editorContent) {
      editorContent = typeof profileData.editorContent === 'string'
        ? JSON.parse(profileData.editorContent)
        : profileData.editorContent;
    } else if (userData?.profile?.editorContent) {
      editorContent = typeof userData.profile.editorContent === 'string'
    ...
    return NextResponse.json({
      ...
      nodeDetails: editorContent ? Object.entries(editorContent).map(([key, node]: [string, any]) => ({
        key,
