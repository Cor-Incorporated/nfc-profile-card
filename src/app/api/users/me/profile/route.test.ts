...
describe("PATCH basic profile atomic sync", () => {
  ...
  it("commits user and public profile address together", async () => {
    ...
    expect(content.email).toBe("public@example.test");
    expect(content.phone).toBe("000-0000-0000");

    const secondResponse = await PATCH(request);
    expect(secondResponse.status).toBe(200);
    expect(
      formatProfileAddress((state.profile.components as any[])[0].content),
    ).toBe("100-0001 東京都千代田区千代田2-2");
  });
