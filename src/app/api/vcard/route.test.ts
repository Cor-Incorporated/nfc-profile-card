...
jest.mock('vcards-js', () => {
  return jest.fn(() => ({
    ...
    getFormattedString: jest.fn(() => 'BEGIN:VCARD\nVERSION:3.0\nN:Doe;John;;;\nFN:John Doe\nEND:VCARD'),
  }))
})
...
describe('VCard API Routes', () => {
  ...
  describe('POST /api/vcard', () => {
    it('基本的なVCardを生成できる', async () => {
      ...
      expect(response.headers.get('Content-Disposition')).toContain('attachment; filename="John_Doe.vcf"')
    ...
    it('完全なプロファイルデータでVCardを生成できる', async () => {
      ...
      expect(vCardInstance.socialUrls.linkedIn).toBe('https://linkedin.com/in/jane')
  ...
  describe('GET /api/vcard', () => {
    it('usernameパラメータでプロファイルを取得してVCardを生成', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/vcard?username=johndoe', {
        method: 'GET',
      })
      ...
      expect(response.status).toBe(200)
      expect(response.headers.get('Content-Type')).toBe('text/vcard;charset=utf-8')
      expect(response.headers.get('Content-Disposition')).toContain('attachment; filename="johndoe.vcf"')
      ...
      expect(vCardInstance.email).toBe('john@example.com')
      expect(vCardInstance.organization).toBe('Example Corp')
      expect(vCardInstance.socialUrls.linkedIn).toBe('https://linkedin.com/in/johndoe')
    })
    ...
    it('X(旧Twitter)のURLを正しく処理する', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/vcard?username=testuser', {
        method: 'GET',
      })
    ...
    it('必須フィールドが存在しない場合でも処理できる', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/vcard?username=emptyuser', {
        method: 'GET',
      })
    ...
    it('プロファイルが見つからない場合404エラーを返す', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/vcard?username=notfound', {
        method: 'GET',
      })
    ...
    it('プロファイルAPI呼び出しが失敗した場合500エラーを返す', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/vcard?username=error', {
        method: 'GET',
      })
    ...
    it('vCard生成中にエラーが発生した場合500エラーを返す', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/vcard?username=erroruser', {
        method: 'GET',
      })
