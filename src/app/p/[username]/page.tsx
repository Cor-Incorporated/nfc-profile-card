...
const fetchUserData = cache(async (username: string) => {
	try {
		...
		try {
			...
			console.error("Failed to read profile subdocument for username:", username, error);
		}
...
export async function generateMetadata({
	...
	return {
		...
		description:
			user.bio || `${user.name}${user.company ? ` | ${user.company}` : ""}${user.position ? ` - ${user.position}` : ""}`,
		openGraph: {
			title: user.name,
...
export default async function ProfilePage({ params }: ProfilePageProps) {
	...
	const vcardLastName = nameParts.length > 1 ? nameParts.slice(1).join(" ") : "";
