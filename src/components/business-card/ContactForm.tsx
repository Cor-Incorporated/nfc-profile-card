...
const ContactForm: React.FC<ContactFormProps> = ({
  ...
  useEffect(() => {
    if (formData) {
      const vcardString = generateVCard(formData, imageBase64 || null, imageMimeType || null);
      setVcardPreview(vcardString);
    }
  }, [formData, imageBase64, imageMimeType]);

  const handleChange = useCallback((field: keyof Omit<ContactInfo, 'phoneNumbers' | 'addresses'>) =>
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setFormData(prev => ({ ...prev, [field]: e.target.value }));
    }, []);

  const handlePhoneChange = (index: number, field: keyof PhoneNumber) =>
    (value: string) => {
  ...
  const handleAddressChange = (index: number, field: keyof Address) =>
    (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const addPhoneNumber = () => {
    setFormData(prev => ({
      ...prev,
      phoneNumbers: [...(prev.phoneNumbers || []), { type: 'WORK', number: '' }]
    }));
  };
  ...
  const addAddress = () => {
    setFormData(prev => ({
      ...prev,
      addresses: [...(prev.addresses || []), { label: '', postalCode: '', address: '' }]
    }));
  };
        ...
        <div className="space-y-3">
          ...
          {(formData.phoneNumbers || []).map((phone, index) => (
            <div key={index} className="flex items-center gap-2">
              <Select
                value={phone.type}
                onValueChange={(value) => handlePhoneChange(index, 'type')(value as any)}
              >
