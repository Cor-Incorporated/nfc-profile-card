...
async function healthCheck() {
  ...
  try {
    ...
    for (const doc of usersSnapshot.docs) {
      ...
      if (userData.profile) {
        ...
        if (userData.profile.editorContent) {
          ...
          if (contentType !== 'string' && contentType !== 'object') {
            results.warnings.push(`User ${userId}: Invalid editorContent type (${contentType})`);
          }
          ...
          try {
            ...
            if (content.ROOT) {
              ...
              results.warnings.push(`User ${userId}: No ROOT node in editorContent`);
            }
          } catch (e) {
            results.warnings.push(`User ${userId}: Invalid JSON in editorContent`);
          }
        }

        // socialLinksの確認
        if (userData.profile.socialLinks) {
          console.log(`    - Has ${userData.profile.socialLinks.length} social links`);
        }
      ...
      try {
        const profilesSnapshot = await db.collection('users').doc(userId)
          .collection('profiles').limit(5).get();

        if (!profilesSnapshot.empty) {
          console.log(`  - Has new profiles structure (${profilesSnapshot.size} profiles)`);
      ...
      if (userData.profileMigrated) {
        ...
        console.log(`  - Migration date: ${userData.migrationDate?.toDate?.() || 'unknown'}`);
      } else {
  ...
  try {
    ...
    for (const doc of usersSnapshot.docs) {
      ...
      // 複数の構造を持つ場合は警告
      const structureCount = [hasLegacyProfile, hasNewProfiles, hasIntermediateProfile]
        .filter(Boolean).length;

      if (structureCount > 1) {
        inconsistentUsers++;
        results.warnings.push(`User ${userId}: Has ${structureCount} different profile structures`);
      }
    }
    ...
    if (inconsistentUsers === 0) {
      ...
      console.log(`  ⚠️ ${inconsistentUsers} users have inconsistent data structures`);
    }
